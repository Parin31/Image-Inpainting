im_face=(imread("..\\images\\baby.jpg"));
im_face_gnoisy=imnoise(im_face,'gaussian');
im_face_snpnoisy=imnoise(im_face,'salt & pepper');
% result=im2double(im_face_snpnoisy);
result=im2double(im_face_gnoisy);
% sz=size(im_face_snpnoisy);
sz= size(im_face_gnoisy);

figure;
imshow(im_face_gnoisy,[]);
% figure;
% imshow(im_face_snpnoisy,[]);
figure;
filter = fspecial('gaussian',5,3);
normalfilter = imfilter(result,filter);
imshow(normalfilter,[]);

for t_step=1:5
    [gx1,gy1]=imgradientxy(result(:,:,1),'sobel');
    [gx2,gy2]=imgradientxy(result(:,:,2),'sobel');
    [gx3,gy3]=imgradientxy(result(:,:,3),'sobel');
    for i=2:sz(1)-1
        for j=2:sz(2)-1
